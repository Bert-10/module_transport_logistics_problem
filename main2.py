...
def fill_optimization_func_frac(d_model: dict, transport: dict, product_name: str, transport_name: str, stock_name: str,
...
def fill_optimization_func_whole(d_model: dict, transport: dict, product_name: str, transport_name: str,
                                 stock_name: str,
                                 need_name: str, row: int, column: int) -> dict:
    last_c_index = d_model['last_c_index']
    b_u = d_model['b_u']
...
def fill_matrix_array(product: dict, matrix_array: list[list[float]], d_model: dict, fill_func) -> list[
    list[float]]:
...
def fill_matrix_array_frac(matrix_array, v_index, c_index, product_name, transport_name, gk, time, stock_name,
...
def solve_problem(data: dict) -> list[dict]:
    ...
    for plan in data['plan_list']:
        ...
        if plan['model'] == 'fractional':
            def_optimization_func = fill_optimization_func_frac
            def_matrix_array = partial(fill_matrix_array_frac, big_number=big_number)
        else:
            def_optimization_func = fill_optimization_func_whole
            def_matrix_array = fill_matrix_array_whole
        ...
        for name in plan['products_names']:
            ...
            # matrix_array = fill_matrix_array(product, matrix_array, d_model, fill_matrix_array_frac)
